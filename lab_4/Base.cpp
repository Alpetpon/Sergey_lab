...
Base::Base(Base&& other) : pBase(nullptr), count(0), capacity(0) {
...
void Base::clear() {
    delete[] pBase; // Освобождаем память, выделенную для массива пар
    pBase = nullptr; // Устанавливаем указатель на nullptr
    count = 0; // Сбрасываем количество пар
    capacity = 0; // Сбрасываем емкость базы данных
}